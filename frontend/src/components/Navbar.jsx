...
import { Search, Heart, Home, Menu, User, LogOut, MapPin, Globe, PlusCircle, ClipboardList, ShieldCheck, MessageCircle } from "lucide-react";
      ...
      <div className="w-full">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                {/* Owner: Nút đăng tin nổi bật */}
                {isOwner && (
                  <Link
                    to="/dashboard/post-property"
                    className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-[20px] text-[11px] font-black hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all hover:-translate-y-0.5 active:scale-95 uppercase tracking-wider"
                  >
                ...
                {isAdmin && (
                  <Link
                    to="/admin"
                    className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-[20px] text-[11px] font-black hover:bg-slate-800 shadow-lg transition-all hover:-translate-y-0.5 active:scale-95 uppercase tracking-wider"
                  >
                ...
                <Link 
                  ...
                  className="p-3 bg-slate-50 text-slate-500 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all relative group"
                >
                  <MessageCircle className="w-5 h-5" />
                  {/* Có thể thêm dot đỏ thông báo tin nhắn chưa đọc ở đây */}
                </Link>
                ...
                >
                  ...
                  <span className="text-[11px] uppercase tracking-wider hidden md:block">Đăng xuất</span>
                </button>
              </div>
            ) : (
